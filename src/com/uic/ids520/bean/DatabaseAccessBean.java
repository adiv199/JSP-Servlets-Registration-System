...
public class DatabaseAccessBean {
	...
	public String processQueryPreparedStatement(String query,List<String> queryValues){
		String status = "FAIL";
		System.out.println("DatabaseAccessBean.processQueryPreparedStatement() query values are "+queryValues);
		System.out.println("DatabaseAccessBean.processQueryPreparedStatement() query is"+query);
		try{
			if(this.connection==null){
				connect();
			}
			...
			try{
				if(this.preparedStatement!=null){
					...
					if(this.resultSet!=null){
						...
						this.resultSet.last();
						rowCount = this.resultSet.getRow();
						System.out.println("DatabaseAccessBean.processQueryPreparedStatement() rowcount is "+rowCount);
						this.resultSet.beforeFirst();
						status = "SUCCESS";
					}
				}
	...
	public String createPrepareStatement(String queryType,String table,List<String> columns,List<String>constraints,Map<String,String>conditionMap){
		if(queryType!=null && !queryType.trim().equals("")){
			...
			System.out.println("DatabaseAccessBean.createPrepareStatement() query is "+query);
			return query;
		}
		return null;
	}
